...
function StatCard({ label, value, sub, accent, bg }: StatCardProps) {
  return (
    <div className={`rounded-2xl ${bg} border border-gray-200 px-5 py-4 flex flex-col gap-0.5 min-w-[100px]`}>
      ...
      <span className={`text-3xl font-bold leading-tight ${accent}`}>
        {value}
      </span>
      {sub && (
  ...
  return (
    <div
      ...
      className="
        ...
        px-8 py-6 cursor-text
        font-mono text-[1.35rem] leading-[2.4rem] tracking-wide
        min-h-[9rem] select-none overflow-hidden
    ...
    >
      {chars.map((c, i) => {
        ...
        let charClass = "text-gray-300"; // pending
        if (c.state === "correct")
          charClass = "text-emerald-600";
        if (c.state === "incorrect")
          charClass =
            c.char === " "
              ? "bg-red-200 text-red-500 rounded-sm"   // space errors get bg
              : "text-red-500 bg-red-50 rounded-sm";

        return (
          <span key={i} className="relative">
            {/* Blinking cursor beam */}
            {isCursor && (
              <span
                ref={cursorRef}
                className="
                  absolute -left-px top-[2px] bottom-[2px] w-[2px]
                  bg-emerald-500 rounded-full animate-pulse z-10
                "
              />
            )}
            <span className={`transition-colors duration-75 ${charClass}`}>
              {/* Render space as non-breaking so it has width + bg shows */}
              {c.char === " " ? "\u00A0" : c.char}
            </span>
          </span>
        );
...
export default function PracticePage() {
  ...
  // Current typed value (reconstruct for controlled input)
  const typedValue = engine.chars
    ...
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <StatCard
          ...
          sub={`${engine.errorCount} error${engine.errorCount !== 1 ? "s" : ""}`}
          accent={accColor}
          bg="bg-white"
        />
        <StatCard
          label="Time"
          value={`${engine.elapsedSeconds}s`}
          sub={engine.isStarted ? "elapsed" : "waiting…"}
          accent="text-rose-500"
          bg="bg-rose-50"
        />
        <StatCard
          label="Progress"
          value={`${engine.cursor}/${targetText.length}`}
          sub="characters"
